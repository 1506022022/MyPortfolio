...
># 파이프 라인
```
프로젝트를 시작하며 가장 먼저 프레임워크를 어떻게 설계할지 고민해 봤습니다.
처음에는 모듈화에 집중해서 트리 구조의 프레임워크를 설계해 봤습니다.
하위 노드에 해당하는 클래스가 부모 노드의 정보를 모르게 하여 커플링을 줄이도록 한 의도는 달성했지만, 트리 구조에서의 문제점을 발견했습니다.
클래스의 상속처럼 파고 들어가는 구조이다 보니 점점 관리가 어려워 질 수 있겠다는 느낌을 받았습니다.

때문에 상속 보다는 컴포지션 구조를 적용하는 편이 좋겠다고 판단했습니다.
그래서 최종적으로는 파이프라인들의 협력 구조로 프레임워크를 설계했습니다.
