...
```
  ## Role
```
 Role 클래스는 축구에서 플레이어와 같은 역할을 담당하는 클래스입니다. 포메이션으로부터 위임받은 행동을
수행합니다. 현재의 위치에서 지정된 포메이션의 위치로 이동하는 하나의 행동을 담당하고 있습니다. 축구에서
플레이어가 이동하는 방식은 사람마다 다를 수 있습니다. 뛰어서 이동하거나 슬라이딩해서 이동할 수도 있습니다.
이렇듯 다양한 이동을 처리하기 위해 움직임을 담당하는 MovementTransform 클래스에 위임했습니다.
```
  ## 코드
``` C#
